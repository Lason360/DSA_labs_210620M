...
struct HashTable{
	...
	//add an if condition to complete the code here
	    if(empty){
	        ...
	        cout<<"Collision Occured. Resolution required!!!";
	    }
	...
	//add an if condition to complete the code here
        if(empty){
            ...
            cout<<"User found: "<<password[hash]<<endl;
        }
    }
...
int main(){
    ...
    while (command!=-1){
        ...
        cin >> stringCommand;
        command = stoi(stringCommand);
        if(command != -1 && command != 1 && command != 2 && command != 3 && command != 4 ){
